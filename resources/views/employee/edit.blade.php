...
@section('contents')
    <div class="info-container">
        <form id="new-employee-form" action="{{ route('employee.save') }}" method="POST"class="new-employee" enctype="multipart/form-data">
            @csrf
                ...
                <div class="upload-con">
                    <div class="upload">
                        ...
                        @if ($employee->image != null)
                            <img src="{{$employee->image}}" alt="" id="preview" style="display: block">
                            <input type="file" name="image" id="image" accept="image/*" value="{{$employee->image}}">
                        @else
                ...
                <div class="personal-info">
                    ...
                    <div class="name input-flex2">
                        <div class="input">
                            ...
                            <input type="text" name="first_name" id="firstname" class="firstname" required value="{{$employee->first_name}}">
                        </div>
                        <div class="input">
                            <label for="lastname">Last Name</label>
                            <input type="text" name="last_name" id="lastname" class="lastname" required value="{{$employee->last_name}}">
                        </div>
                        <div class="input">
                            <label for="middlename">Middle Name</label>
                            <input type="text" name="middle_name" id="middlename" class="middlename" required value="{{$employee->middle_name}}">
                        </div>
                    </div>
                    <div class="other-personal-info input-flex2">
                        <div class="input">
                            <label for="birth">Date of Birth</label>
                            <input type="date" name="birth" id="birth" class="birth" require value="{{$employee->birthdate}}"d>
                        </div>
                        <div class="input">
                            <label for="nationality">Nationality</label>
                            <input type="text" name="nationality" id="nationality" class="nationality"required value="{{$employee->nationality}}">
                        </div>
                        <div class="input">
                            <label for="gender">Gender</label>
                            <input type="text" name="gender" id="gender" class="gender" required value="{{$employee->gender}}">
                        </div>
                    </div>
                </div>
            </section>
                ...
                <div class="job-description">
                    ...
                    <div class="job input-flex2">
                        <div class="input">
                            ...
                            <input type="text" name="position" id="position" class="position" required value="{{$employee->position}}">
                        </div>
                        <div class="input">
                            <label for="job-type">Job Type</label>
                            <input type="text" name="job_type" id="job-type" class="job-type" required value="{{$employee->job_type}}">
                        </div>
                    </div>
                    <div class="date input-flex2">
                        <div class="input">
                            <label for="date-hired">Date Hired</label>
                            <input type="date" name="date-hired" id="date-hired" class="date-hired" required value="{{$employee->hired_date}}">
                        </div>
                        <div class="input">
                            <label for="start-date">Start Date</label>
                            <input type="date" name="start-date" id="start-date" class="start-date" required value="{{$employee->start_date}}">
                        </div>
                    </div>
                    <div class="input">
                        <label for="supervisor">Supervisor</label>
                        <input type="text" name="supervisor" id="supervisor" class="supervisor" required value="{{$employee->supervisor}}">
                    </div>
                    <div class="rate-id input-flex2">
                        <div class="input">
                            <label for="daily-rate">Daily Rate (₱)</label>
                            <input type="number" name="daily-rate" id="daily-rate" class="daily-rate" required value="{{$employee->daily_rate}}">
                        </div>
                        <div class="input">
                            <label for="employee-id">Emplyoee ID</label>
                            <input type="number" name="employee_id" id="employee-id" class="employee-id" required value="{{$employee->employee_id}}" readonly>
                        </div>
                    </div>

                    <h1>Total Cash Advance Balance (₱)</h1>
                    <div class="input">
                        <input type="number" name="cab" id="cab" class="cab" required value="{{$employee->cash_advance}}">
                    </div>
                </div>
                <div class="contact-information">
                    ...
                    <div class="number-email input-flex2">
                        <div class="input">
                            ...
                            <input type="number" name="contact_number" id="contact-number" class="contact-number" required value="{{$employee->contact_number}}">
                        </div>
                        <div class="input">
                            <label for="email">Email</label>
                            <input type="email" name="email" id="email" class="email" required value="{{$employee->email}}">
                        </div>
                    </div>
                    <div class="input">
                        <label for="address">Address</label>
                        <input type="text" name="address" id="address" class="address" required value="{{$employee->address}}">
                    </div>
